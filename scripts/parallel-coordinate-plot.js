...
function toggleMassReference(checked) {
  ...
  if (!checked) {
    ...
    jupiterReferences.forEach((reference) => {
      allDimensions.splice(
        allDimensions.indexOf(reference),
        1,
        earthReferences[jupiterReferences.indexOf(reference)],
      );
      if (activeDimensions.indexOf(reference) !== -1) {
        activeDimensions.splice(
          activeDimensions.indexOf(reference),
          1,
          earthReferences[jupiterReferences.indexOf(reference)],
        );
      }
    ...
    earthReferences.forEach((reference) => {
      allDimensions.splice(
        allDimensions.indexOf(reference),
        1,
        jupiterReferences[earthReferences.indexOf(reference)],
      );
      if (activeDimensions.indexOf(reference) !== -1) {
        activeDimensions.splice(
          activeDimensions.indexOf(reference),
...
function createCheckboxes() {
  ...
  allDimensions.forEach((dim) => {
    ...
    const dimName = columnExplanations[dim]
      ? columnExplanations[dim].name
      : dim;
    label.append("span").text(" " + dimName);

    // Setup tooltip if data is already loaded
    if (columnExplanations[dim]) {
      label.attr(
        "title",
        columnExplanations[dim].name + ": " + columnExplanations[dim].desc,
      );
    }
  });
}
...
function formatDimensionValue(dim, value) {
  ...
  if (dim === "disc_year" || isIntegerCountField(dim))
...
Promise.all([
  ...
  if (data.length > MAX_LINES) {
    if (warningBanner) {
      ...
      warningBanner.innerHTML = `⚠️ Showing a sample of ${MAX_LINES} planets (out of ${data.length} selected) to maintain performance.`;
    }
    // Shuffle and slice
    d3.shuffle(data);
...
function draw(dimensions) {
  ...
  dimensions.forEach((dim) => {
    ...
    if (shouldUseLog) {
      y[dim] = d3.scaleLog().domain([min, max]).range([height, 0]).nice();
      scaleTypes[dim] = "LOG";
    } else {
      y[dim] = d3.scaleLinear().domain([min, max]).range([height, 0]).nice();
      scaleTypes[dim] = "LIN";
    }
  });
  ...
  const g = svg
    ...
    .each(function (dim) {
      d3.select(this).call(createAxis(dim));
    });
  ...
  g.append("text")
    ...
    .on("mouseover", function (event, d) {
      ...
      getTooltip().html(content).style("visibility", "visible");
    })
  ...
  d3.selectAll(".brush-max, .brush-min").on("change", function (event) {
    ...
    if (isNaN(maxVal) && isNaN(minVal)) {
      ...
      d3.select(brushGroup).call(brushGroup.__brush.move, [
        Math.min(y0, y1),
        Math.max(y0, y1),
      ]);
    }
  ...
  function updateLines() {
    svg.selectAll(".line-group").style("display", function (d) {
  ...
  function distanceToLine(points, px, py) {
    ...
    for (let i = 1; i < points.length; i++) {
      minDistance = Math.min(
        minDistance,
        distanceToSegment(px, py, points[i - 1], points[i]),
      );
    }
    return minDistance;
  }
  ...
  function distanceToSegment(px, py, a, b) {
    ...
    const t = Math.max(
      0,
      Math.min(1, ((px - a[0]) * dx + (py - a[1]) * dy) / lengthSquared),
    );
  ...
  const hoverHaloPath = hoverLayer
    .append("path")
  ...
  const hoverDashedPath = hoverLayer
    .append("path")
    .attr("class", "line-dashed")
    .style("fill", "none")
    .style("stroke-dasharray", "4 4")
  ...
  const hoverSolidPath = hoverLayer
    .append("path")
    .attr("class", "line-solid")
    .style("fill", "none")
    .style("stroke-linecap", "round")
  ...
  hoverRenderer = {
    focus(d) {
      ...
      hoverHaloPath.attr("d", solidPath);
      hoverDashedPath.attr("d", dashedPath).style("stroke", stroke);
      hoverSolidPath.attr("d", solidPath).style("stroke", stroke);
    },
    clear() {
  ...
  selectedValueRenderer = {
    render(d) {
      ...
      const markers = selectedValueLayer
        ...
        .join(
          (enter) => {
            ...
            marker.append("circle").attr("r", 4.5);
            ...
            marker.append("text").attr("x", 8).attr("y", -8);
      ...
      markers.attr(
        "transform",
        (item) => `translate(${position(item.dim)},${y[item.dim](item.value)})`,
      );
      ...
      markers
        ...
        .attr("text-anchor", (item) =>
          position(item.dim) > width - 70 ? "end" : "start",
        )
      ...
      const firstSelectedPoint = selectedPoints.length
        ? selectedPoints[0]
        : [0, 18];
      const namePoint = [
        -margin.left + 8,
        Math.max(20, Math.min(height - 18, firstSelectedPoint[1])),
      ];
      ...
      const nameEnter = nameLabel
        .enter()
        .append("g")
        .attr("class", "selected-planet-name");

      nameEnter.append("rect").attr("rx", 7).attr("ry", 7);
      ...
      mergedNameLabel.attr("transform", () => {
      ...
      mergedNameLabel.each(function () {
        ...
        label.select("text").attr("x", offsetX + 9);
    ...
    if (exists) {
      lineGroups
        .classed("selected", function (d) {
          return d.pl_name === selectedPlanet.pl_name;
        })
        .classed("dimmed", function (d) {
          return d.pl_name !== selectedPlanet.pl_name;
        });
      const currentSelected = data.find(
        (d) => d.pl_name === selectedPlanet.pl_name,
      );
...
function clearHoverFocus(force = false) {
  ...
  const clear = () => {
    ...
    if (selectedPlanet) {
      d3.selectAll(".line-group")
        .classed("selected", function (lineData) {
          return lineData.pl_name === selectedPlanet.pl_name;
        })
        .classed("dimmed", function (lineData) {
          return lineData.pl_name !== selectedPlanet.pl_name;
        });
    } else {
      d3.selectAll(".line-group").classed("dimmed", false);
    }
  };
...
function selectDataline(d) {
  ...
  d3.selectAll(".line-group")
    .classed("selected", function (lineData) {
      return lineData.pl_name === d.pl_name;
    })
    .classed("dimmed", function (lineData) {
      return lineData.pl_name !== d.pl_name;
    });
