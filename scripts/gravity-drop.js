...
d3.csv("../data/nasa_export_small.csv", (d) => {
  ...
  if (state.data.length > 0) {
    let defaultPlanet =
      state.data.find((d) => d.name === "HD 209458 b") || state.data[0];
...
function onPlanetSelect(planetName) {
  ...
  d3.select("#val-mass").text(
    `${state.selectedPlanet.mass.toFixed(2)} M_earth`,
  );
  d3.select("#val-rad").text(`${state.selectedPlanet.rad.toFixed(2)} R_earth`);
  d3.select("#val-grav").text(
    `${state.gExo.toFixed(2)} m/s² (${state.selectedPlanet.gravityRel.toFixed(2)}g)`,
  );
  d3.select("#exo-title")
    .text(state.selectedPlanet.name)
    .append("span")
    .attr("class", "sub")
    .text(` (${state.selectedPlanet.gravityRel.toFixed(2)}g)`);
